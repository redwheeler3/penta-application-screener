...
The per-pass `given`/`expected` shapes match what each pass's real `build_prompt` and output
schema send and return. The judge owns no case files; it reads the per-pass golden files through
this shared schema.
...
| **screening** | `{ fires: [category, …], absent: [category, …], contested?: bool, pets?: { dogs, cats, other_pets } }` — categories that MUST fire / must NOT (over-reach guards); a clean applicant has empty `fires`/`absent` and no `pets`, and any flag fails it. A `fires` entry may itself be a **list** = "at least ONE of these must fire" (for a concern with more than one defensible bucket) — or, as input sugar, the pipe-delimited string the eval DISPLAYS that group as (`"spam_essay | minimal_essay"`), which the loader normalizes to the list (`_normalize_fires`), so what you see is what you can type. **`contested`** is a BOOL, exactly like the categorical passes carry: `fires`/`absent`/`pets` still define the expectation and are graded normally, but `contested: true` says a MISS on this case is expected/defensible — so the failure reads AMBER not red, and a stability flip reads `[contested-split]` not `[UNSTABLE]`. It never changes what "met" means (grading is identical); it only changes how a *failure* is presented. Use for a genuine judgment call reasonable screeners split on (e.g. `fires: [[spam_essay, minimal_essay, ai_generated_essay]]` + `contested: true` — we expect a vapid buzzword essay caught, but not catching it is a defensible "weak-but-sincere" read, not a regression). Green when met, amber when missed, never red. (A screening case is multi-axis, so a contested axis can't fail the case, but the case's real `fires`/`absent`/`pets` assertions still bite.) **`pets`** grades the model's neutral pet EXTRACTION, not a flag: `dogs`/`cats` must match exactly and each `other_pets` noun must appear (substring, case-insensitive) in the extracted `other_pets`. A case that sets `pets` is not "clean" (it has a positive expectation), so an incidental flag doesn't fail it unless it's in `absent`. Pets are never a flag category — the per-member pet limit is judged deterministically downstream, not by the model. The model also emits a neutral one-sentence `pets.reasoning`; like a flag's summary it is prose and is NOT graded — only the counts are. |
