...
| **screening** | `{ fires: [category, …], absent: [category, …], contested?: [category, …], pets?: { dogs, cats, other_pets } }` — categories that MUST fire / must NOT (over-reach guards); a clean applicant has empty `fires`/`absent`/`contested` and no `pets`, and any flag fails it. A `fires` entry may itself be a **list** = "at least ONE of these must fire" (for a concern with more than one defensible bucket) — or, as input sugar, the pipe-delimited string the eval DISPLAYS that group as (`"spam_essay | minimal_essay"`), which the loader normalizes to the list (`_normalize_fires`), so what you see is what you can type. **`contested`** (M15) NAMES the flag categories that could defensibly fire or not — a tone/quality call reasonable screeners split on. A contested category is neither required nor forbidden (exempt from the over-reach + clean-case rules), and — the point of naming it rather than just omitting it — the eval DISPLAYS it (`contested: …`) and treats it visually like the categorical passes' `contested` bool. The EXPECTATION is that one of the contested categories fires (the concern we want caught), so: on a normal run the dot is green when the model fired one (met the expectation) and amber when it caught none (defensible, but the debated concern went unflagged); in a stability run a rep that fired one tokens `pass`, a rep that caught none tokens `fail`, and the run-to-run wobble reads `[contested-split]`, not `[UNSTABLE]`. Never reds on the contested axis. (A screening case is multi-axis, so a contested axis can't fail the case, but the case's real `fires`/`absent`/`pets` assertions still bite.) **`pets`** (optional, M15 1e) grades the model's neutral pet EXTRACTION, not a flag: `dogs`/`cats` must match exactly and each `other_pets` noun must appear (substring, case-insensitive) in the extracted `other_pets`. A case that sets `pets` is not "clean" (it has a positive expectation), so an incidental flag doesn't fail it unless it's in `absent`. Pets are never a flag category — the per-member pet limit is judged deterministically downstream, not by the model. The model also emits a neutral one-sentence `pets.reasoning` (the reading shown as the pet finding's evidence, M15 1g); like a flag's summary it is prose and is NOT graded — only the counts are |
