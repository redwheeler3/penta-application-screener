...
This document explains how AI-assisted screening works: the **screening pass** that reviews applications for data-integrity concerns and the **Rank chain** that discovers evaluation criteria across one selected opening's pool and scores candidates against them, how results are cached and cost-capped, and how each pass runs many applications concurrently.
...
class PetFacts(BaseModel):      # neutral extraction, never a verdict
    dogs: int = 0
    cats: int = 0
...
- **`prompt_version`** — **derived by hashing each pass's static prompt text** (`derive_prompt_version` in `analysis.py`), not bumped by hand: editing the prompt or an embedded fragment re-derives it automatically, so results from a different prompt are not reused. Screening's `screening_prompt_version()` is a pure function of its prompt text. The prompt extracts neutral pet facts without receiving the pet policy, so a pet-limit change is a hard-filter change judged on read rather than a screening-cache change.
...
The frontend surfaces this as a **separate ranked view**, not a re-sort of the browse table: the order is the product, read top-down, and the tier-list maker docks above it (drag criteria into importance tiers → `PUT /ranking/tiers` → re-sort, no model call). Each analysis and member tier layout belongs to one opening. Re-running the Rank chain finds fresh criteria for that opening and re-scores, then refreshes its ranking. The chain is gated on a **rank-inputs fingerprint** (`Analysis.rank_inputs_fingerprint`, its own indexed column — a hash of the opening's pool *plus* each rank-chain prompt, model identity, and applicable reasoning level), so re-ranking with unchanged inputs is flagged up-front (`/ranking/run/estimate` returns `ranking_current: true`); a re-run is still allowed when the opening has no final outcome (discovery is nondeterministic, so a member may want a fresh criteria set — the confirmation card explains nothing requires it). A new/edited/eligibility-changed application in that opening, edited prompt, model change, or reasoning change moves the fingerprint. Switching only between catalogued Bedrock and direct routes for the same pinned model does not. Canonical dimension history and content-addressed scores remain shared across openings, so a genuinely equivalent past criterion can reuse scores without injecting another opening's criteria into discovery.

**The Rank button.** The model passes that produce a ranking — pattern discovery, decomposition, identity-match, dimension scoring, and post-score consolidation — are exposed in the UI as a single "Rank" step, because the committee never runs them individually. `POST /ranking/run` orchestrates them back-to-back and streams phase-aware progress (`phase` lines for criteria / scores / consolidate, `progress` lines within the per-candidate phases, a final `summary`). The passes stay separate underneath with distinct schemas, cache kinds, and status behavior. The cap is enforced **once over the combined projected cost** (`GET /ranking/run/estimate` sums discovery + decomposition + scoring + consolidation), before any model call. The estimate is approximate: scoring scales with the dimensions discovery settles on, which do not exist until the criteria phase runs, so it is labeled as such. The workflow strip is **Screen** followed by **Rank**; deterministic rules are computed from submitted fields without a separate action.
