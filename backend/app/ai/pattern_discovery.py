...
- **A quantity with NO inherent desirable direction is never a raw score — reframe it to the fit concept(s) it drives.** If a fact varies across the pool but neither "more" nor "less" is inherently better fit (a purely descriptive or matching attribute), do NOT emit it as a directionless score — a 0..1 on it is uninterpretable (is 0.8 more or less?) and weighting it weights noise. Instead name the specific fit concept(s) the quantity DRIVES, each oriented more-is-better and grounded in the co-op criteria — and emit several if several apply, even conflicting ones (the committee picks). Do NOT try to split the raw quantity into "large" vs. "small" versions of itself; a single number read two directions does not split — reframe to the underlying concepts, or, if none genuinely apply, drop it (a bare descriptor the committee can read off the facts is not a scored dimension). (Out-of-domain illustration, do not borrow the subject: "vehicle weight" has no good direction on its own — reframe to what it drives: "cargo capacity" (heavier is better) and "fuel efficiency" (lighter is better), two oriented axes, not one directionless "weight" score.) This is the same instinct as the Goldilocks rule below, applied to a flat-preference quantity rather than a peaked one.
