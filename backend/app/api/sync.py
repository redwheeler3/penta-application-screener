...
@router.post("/applications", response_model=SyncResponse)
def sync_applications(
    ...
    if not rows:
        raise Problem(
            "google_sheet_empty",
            detail="Google Sheet returned no data rows. Check that the sheet has a header row and at least one data row.",
        )
    ...
    return SyncResponse(
        ...
        duplicate_count=sync_run.duplicate_count,
        imported_count=sync_run.imported_count,
        updated_count=sync_run.updated_count,
        unchanged_count=sync_run.unchanged_count,
        eligible_count=sync_run.eligible_count,
        filtered_out_count=sync_run.filtered_out_count,
    )
