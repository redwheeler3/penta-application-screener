...
def _run(db: Session, *, report_keys: list[str], tiers: list[dict]) -> RankingRun:
    """A minimal run carrying a dimension report + stored (working) tiers. Ignore is
    every report key not in a working tier — never stored, matching production."""
    run = RankingRun(dimension_report=_report(*report_keys), run_state={"tiers": tiers})
    db.add(run)
    db.commit()
    return run
...
def test_recent_ignore_beats_older_working_placement() -> None:
    ...
    _scaffold, tier_by_key = tier_history(db)
...
def test_all_ignore_board_stays_all_ignore_on_rerank() -> None:
    ...
    scaffold, tier_by_key = tier_history(db)
