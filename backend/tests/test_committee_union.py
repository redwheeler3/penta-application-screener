...
def add_analysis(db: Session, *keys: str) -> Analysis:
    analysis = Analysis(
        ...
        dimension_report=report(*keys).model_dump(mode="json"),
    )
    db.add(analysis)
    db.flush()
    return analysis
...
def test_axis_survives_if_any_member_tiered_it() -> None:
    ...
    kept = committee_kept_keys(db, current_opening_id(db), report("community", "skills"))
...
def test_ignored_axis_is_not_kept() -> None:
    ...
    assert committee_kept_keys(
        db, current_opening_id(db), report("community", "skills")
    ) == {"community"}
...
def test_kept_restricted_to_current_report_keys() -> None:
    ...
    assert committee_kept_keys(db, current_opening_id(db), report("community")) == {"community"}


def test_kept_empty_on_first_run() -> None:
    db = make_db()
    add_user(db, "a@x.com")
    assert committee_kept_keys(db, current_opening_id(db), None) == set()
