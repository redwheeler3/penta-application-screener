...
1. Data-model split, sliced into four independently-shippable, separately-migrated steps (the maps confirmed the `RankingRun` write side funnels through 4 service functions, but ~10 "BOTH" functions join member-view to shared-analysis, and `status == ELIGIBLE` is the pool filter in four independent places — so slicing keeps each migration small and round-trip-verifiable):
   ...
   - **1e — pet policy as deterministic per-member facts (option B).** The map showed pets are judged inside the shared screening prompt (a `pet_policy` *flag*), not extracted as facts — so making them per-member is a screening-pass redesign, not a settings move, and earns its own slice. 1e: screening extracts raw pet *facts* (counts/types) as neutral evidence; a new per-member pet hard-filter in `RulesConfig` decides pass/fail over those facts (like every numeric rule) — moving pet judgment out of AI-arithmetic and letting pet limits diverge per member without fragmenting the shared screening cache. Requires re-baselining `screening_prompt_version` + touching eval fixtures. Once done, pet limits move from `AppSettings` into `EligibilityRules` (per-member) — completing the rules split.
