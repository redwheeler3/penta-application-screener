...
1. Data-model split, sliced into four independently-shippable, separately-migrated steps (the maps confirmed the `RankingRun` write side funnels through 4 service functions, but ~10 "BOTH" functions join member-view to shared-analysis, and `status == ELIGIBLE` is the pool filter in four independent places — so slicing keeps each migration small and round-trip-verifiable):
   ...
   - **1f — admin-editable committee defaults + member reset-to-default** (sub-milestone after 1e). Two gaps 1d left: (a) the committee-default rules are only *seeded* by migration — no in-app way to change them; add an admin-gated edit (in Admin Settings) so an admin can update the baseline every non-diverged member reads. (b) A member who diverged has a `MemberRules` row forever — `PUT /eligibility-rules` only upserts; add `DELETE /eligibility-rules` + a "Reset to committee default" action so a member can drop their divergence and follow the committee again (restores `isDefault`).
