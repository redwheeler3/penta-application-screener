...
**"New" is a transient annotation, not a tier.** An unmatched dimension sitting in Ignore is **badged "new"** (a colour/marker — exact treatment TBD in build) so it is silent to the *math* (weight 0) but loud to the *human* (clearly flagged as needing a decision). Badge lifecycle: a chip shows "new" iff its key ∈ `new_dimension_keys` **and** it is still in Ignore. Dragging it into any working tier (S/A/B) triages it → badge clears. Leaving it in Ignore keeps the badge (harmless, and honestly useful: "new *and* parked"). `new_dimension_keys` is recomputed on each re-discovery; it needs only one stored set per run, no per-chip "seen" flag.
