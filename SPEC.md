...
>
> **Discovery surfaced (2026-07-07).** The Insights tab's `DiscoveryPanel` now shows the discovery half of the run-level axis: the pool summary, the full dimension set (each with definition + why-it-differentiates + name/key, "new" badged), and — in a nested disclosure — the model's full streamed **reasoning** (`criteria.discovery_narrative`, rendered as markdown). Exposed via a `discovery_narrative` field on `GET /ranking/current` (no new endpoint — the panel already receives that payload). Verified live against run 14's real reasoning.
>
> **Still open for Stage 1: the per-application axis** — extend the candidate detail page with trace metadata (tokens/cost/model/version) on the existing screening + essay sections, and add the missing dimension-scoring trace.
