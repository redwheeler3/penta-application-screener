...
**Applying the mapping — one rule, no "New" bucket.** After the match pass:
- A **matched** new dimension inherits the tier its old counterpart was in. (Auto-applied.)
- An **unmatched** new dimension (genuinely new, or a survivor the high bar declined to match) lands in **Ignore**, and its key is added to the run's `new_dimension_keys` set.

There is deliberately **no separate "New" bucket.** A second bucket has undefined semantics the moment the committee leaves something in it ("what do we do with un-triaged 'new' items?"). Putting unmatched dimensions in **Ignore** makes the answer well-defined and *correct by construction*: weight 0 means an un-triaged dimension cannot silently influence the ranking before anyone decided it matters — the same "human decides what matters" principle we protect everywhere. "Leave it alone and we ignore it" is then literally true, not a special rule.
