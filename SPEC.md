...
- *Phase 1 (foundation):* `weights_from_tiers` falls back to uniform when *no dimension has positive weight* (not just when there are zero non-ignore tiers); `default_tier_layout` starts every dimension in **Ignore** (S/A/B empty). (A "nothing weighted yet" baseline label was considered and dropped.)
...
- **Place it in a working tier** (drag to S/A/B) — it's no longer unplaced, so it drops out; this is the "I've decided where it goes" path.
