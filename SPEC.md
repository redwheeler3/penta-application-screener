...
2. **Cost surfacing (Pillar 1).** Aggregation over `ApplicationAIResult` + estimate-vs-actual reconciliation.
...
>
> **Per-application axis surfaced (2026-07-07) — Stage 1 COMPLETE.** The candidate detail page gained an **"AI trace (cost & tokens)"** collapsed panel (beside the existing raw-source / raw-AI / essay-summary panels), rolling up per-pass model / prompt-version / calls / tokens / cost + a total. Placement decided with Jeff: a single operator panel, NOT metadata sprinkled onto the flags/essay/fit-dimension sections (those are the committee's *decision* content; token/cost is *inspection* detail, same category as the Insights tab). Served via an `ai_trace` field on the application detail (no new endpoint). **Correctness note:** dimension scoring is rolled up over the **current run's dimension keys only** — a re-rank leaves `ApplicationAIResult` rows for renamed/dropped dimensions, so summing all `dimension_scoring:*` kinds double-counted stale work (63 calls vs. the real 16); the rollup scopes to `current_dimension_report`'s keys, matching the Fit-dimensions section. `mixed_versions`/`(mixed)` model surface when a rolled-up pass spans >1 prompt version. Verified live (run 15): 1 screening + 1 essay + 16 scoring calls, $0.0325 total.
