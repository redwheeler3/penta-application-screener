...
2. **Add a dimension mid-tiering** (after carry-forward). Dragging dimensions around is exactly when the committee notices a factor the AI missed, so the tier-list grows an "add dimension" affordance. A user-added dimension changes the dimension set → new `dims_hash` → it re-enters the **cap-gated scoring pass for that one dimension** (existing scores stay cached), then appears as a chip. No new AI surface — it reuses the existing scoring pass. This relocates the parked "human-editable criteria" enhancement into the M9 flow where the need actually arises. (Depends on the per-dimension scoring cache described below — once scoring caches per dimension, adding one axis costs one dimension's worth of scoring, not a full re-score.)
...
**Build status:** Phases 1–3 + two hardening passes done and **verified end-to-end against real Bedrock** (ran the full Import→Screen→Rank chain, tiered dimensions, forced a pool change, re-ranked: 6/8 placements carried across drifted keys, an ambiguous match correctly declined, unmatched dims parked in Ignore and badged, badge cleared on triage). Remaining: Phases 4–5 (cost optimization — not yet built).
...
Remaining: Phase 4 (per-dimension score reuse — per-dimension cache rows + one batched call per candidate, keyed by the match mapping), Phase 5 (estimator + confirmation updates for the match-pass cost and reduced scoring). These are **cost optimization only** — carry-forward is functionally complete and hardened through real use without them; today's re-rank still re-scores the whole pool (correct, just not yet cheap).
...
- Per-run scoring cache `kind = "dimension_scoring:<dims-hash>"`; estimate self-tunes across dims-hashes via the `dimension_scoring:` prefix. (Per-*dimension* reuse is Phase 4, not yet built — re-rank still re-scores the whole pool.)
