...
1. Data-model split, sliced into four independently-shippable, separately-migrated steps (the maps confirmed the `RankingRun` write side funnels through 4 service functions, but ~10 "BOTH" functions join member-view to shared-analysis, and `status == ELIGIBLE` is the pool filter in four independent places — so slicing keeps each migration small and round-trip-verifiable):
   ...
   - **1g — restore the two-phase eligibility mental model after pets (design locked 2026-07-24; not yet built).** After 1e made pets a deterministic per-member rule, they surfaced with a **"decided by Rules"** badge — but a pet verdict needs the AI to read the free-text pets field first, so it can only land at **Screen**, not **Sync**. That made the workflow read *"some rules resolve at Sync, one more rule shows up later"* — the appears-≠-is trap. The fix is NOT to relocate any computation (eligibility is compute-on-read and per-member, so there is no single verdict to "run" or store at a step — trying to would re-introduce the write-fanout/stored-verdict mess 1c deleted). The fix is to put pets in the honest bucket. Three moves, sequenced:
     ...
     - **Move 2 — members can toggle the AI checks on/off (not just the deterministic rules).** Today `EligibilitySettingsPanel` only lets a member disable the deterministic rules; the AI checks (internal_inconsistency, fake_contact, etc.) always run. Since eligibility is per-member, AI checks should be per-member too: a disabled category stops counting toward *that member's* `has_ai_flags`. **Cost:** this makes AI flags per-member, which breaks the union pool's current "any flag → ineligible for everyone" shortcut — the union must group by each member's disabled set, the *same* technique 1d used for numeric rules (known work, not novel: union rework + tests + toggle UI). Heavier than Move 1 → **next slice after Move 1.**
     - **Move 3 — rename `disabled_rules` → `disabled_checks` (rides with Move 2).** Once AI flag categories are toggleable alongside numeric rules, `disabled_rules` mildly lies (a flag category isn't a "rule"). Rename to `disabled_checks` across `EligibilityRules` / `committee_default_rules` / `member_rules` (stored-blob migration). Cardinal rule: the name shouldn't lie. Bundle into the Move 2 slice.
