...
- *Phase 3 (new-dimension badging):* the tier-list reads `newDimensionKeys` (from `/current`) and badges each unmatched-new dimension with an amber "New" marker **while it sits in Ignore**; dragging it into a working tier triages it and the badge clears (the badge is `key ∈ newDimensionKeys && tier.ignore`). Threaded `newKeys` through `TierList → TierRow → DimensionChip → ChipBody`. First run flags nothing (empty `newDimensionKeys`).

Remaining: Phase 4 (per-dimension score reuse — per-dimension cache rows + one batched call per candidate, keyed by the match mapping), Phase 5 (estimator + confirmation updates for the match-pass cost and reduced scoring).
...
Milestones 1–9 are complete and proven end-to-end against real Bedrock (sync → quality flags → essays → discover ~14–16 fact-aware dimensions → score the pool → rank with the tier-list weighting). M8 delivered the deterministic ranked list (weighted average over the M7 `DimensionScore`s, equal-weight baseline, relative bands); M9 layered the tier-list maker on top (the committee drags criteria into S/A/B/Ignore tiers and the list re-sorts — no model call). **The next milestone is 10: Google Docs report generation.** Three M9 fast-follows are queued, in order (see the M9 design section): (1) **tier carry-forward on re-rank** — designed in detail under "Tier Carry-Forward On Re-Rank," fixes re-rank silently discarding the committee's tiers, via a blind-discovery + identity-match two-pass with auto-applied high-confidence carry-forward, "everything starts in Ignore," new-dimension badging, a weight-fallback math fix, per-dimension scoring cache, and estimator/confirmation updates; (2) add-a-dimension mid-tiering; (3) the AI Criteria Coach. The current M9 ships with no model call for ranking; carry-forward (fast-follow #1) is the first to add model calls (the match pass) to the re-rank path.
...
The next milestone is **10: Google Docs report generation**; three M9 fast-follows are queued, in order: (1) tier carry-forward on re-rank (designed — see "Tier Carry-Forward On Re-Rank"), (2) add-a-dimension mid-tiering, (3) the AI Criteria Coach.
