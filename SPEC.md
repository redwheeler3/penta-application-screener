...
1. Data-model split, sliced into four independently-shippable, separately-migrated steps (the maps confirmed the `RankingRun` write side funnels through 4 service functions, but ~10 "BOTH" functions join member-view to shared-analysis, and `status == ELIGIBLE` is the pool filter in four independent places — so slicing keeps each migration small and round-trip-verifiable):
   ...
   - **1e — pet policy as deterministic per-member facts (option B).** Pets are judged inside the shared screening prompt (a `pet_policy` *flag*), not extracted as facts — so making them per-member is a screening-pass redesign, not a settings move, and earns its own slice. Move pet judgment out of AI-arithmetic: the shared screening pass extracts raw pet *facts*, and a per-member deterministic hard-filter decides pass/fail — so pet limits diverge per member without fragmenting the shared screening cache. **Implementation (fresh session: re-map exact file:line first — screening.py drifted in 1c/1d):**
     1. **Screening prompt** (`app/ai/screening.py`): drop `_pet_policy_line` + its interpolation into the prompt (`{pet_policy}` placeholder) and its fold into `screening_prompt_version` (via `derive_prompt_version`). Screening stops *judging* pets. Instead the screening output schema (`ScreeningReport`/`ScreeningFlag` in `app/ai/schemas.py`) gains neutral extracted **pet facts** (e.g. `dogs: int, cats: int, other_pets: list[str]`) — a fact the model can ground, not a threshold verdict. Remove the `pet_policy` `FlagCategory` (or keep it only for genuinely un-judgeable cases — decide at build).
     2. **Hard filter** (`app/domain/hard_filters.py`): add pet fields to `RulesConfig` (`max_dogs`, `max_cats`, `allow_other_pets`) + pet sub-checks in `evaluate_hard_filters` over the extracted facts (reads them off `normalized` or the screening result — decide where pet facts live so on-read eligibility can see them). Now pets are per-member exactly like income/age.
     3. **Settings move:** pet fields (`max_dogs`/`max_cats`/`allow_other_pets`) move from `AppSettings` → `EligibilityRules` (per-member; committee default + copy-on-write), completing the rules split. `AppSettings` is then pure infra (sheet + AI). Frontend: pet inputs move from Admin Settings → Eligibility Settings.
     4. **Re-baseline:** `screening_prompt_version` changes (prompt text changed) → the screening cache invalidates and eval fixtures that pin screening output shift. Re-baseline the committed screening eval fixtures/goldens after a **real Bedrock screening run** (mock proves plumbing only — the real risk is whether the model reliably extracts pet counts; validate on real data, tune the prompt from what you observe). This is the slice's real cost + the one that needs Jeff's real-run go-ahead.
   Once done, pet limits are per-member and the rules split is complete.
