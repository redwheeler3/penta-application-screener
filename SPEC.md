...
**Why before observability (M12).** Observability instruments the API (per-endpoint metrics, audit surfacing) and a second screening actor (M13) is a new consumer of it — both are cheaper to build on a clean, consistent surface than to retrofit, then re-instrument, after a later redesign. Sequencing the redesign first avoids doing the wiring twice.

### Observability And Evals (Milestone 12)

The pipeline now makes real, non-deterministic model judgments (discovery, scoring, the carry-forward match pass), and we have already hit the wall their opacity creates: when a re-rank carried 18/18 organic dimensions forward by identical key, we could not tell *genuine re-discovery* from *match-pass over-matching* — because the stored run kept only the rewritten result, not what discovery actually emitted or how the match pass reasoned. That gap is the motivation: **M12 makes the AI legible — what it cost, what it did, and whether it is any good.** The first brick already exists (`criteria.match_audit`: raw pre-adopt discovery dims + the new→old match map + the match narrative); M12 generalizes that instinct across all four pillars.
...
> "
> ```
> Building the API endpoint + admin viewer for this is a concrete, ready-to-pick-up task whenever M12 starts.
...
12. AI observability: evals, metrics, cost, and audit surfacing (see "Observability And Evals").
13. Multi-member screening and merged shortlist comparison.
