...
> **SURFACED (Stage 1 run-level axis — done 2026-07-07).** `match_audit` is no longer a SQLite spelunk. `GET /ranking/current/match-audit` returns the raw-discovery dimensions, the new→old map, the match narrative, and the derived counts + **carry-forward rate** (`match_audit_view` in `ranking_run.py`); the `MatchAuditPanel` on the ranked view renders it as a collapsible debug panel, colouring the rate amber (≥60%) / red (≥90%) toward the over-matching band. One small capture was added — `prior_dimension_count` — so the rate is null on a genuine first run (undefined) rather than guessed from an empty map; runs written before this field render as "first run" until re-ranked (no shim, per the disposable-DB rule). Verified live against real Bedrock: two consecutive re-runs sat at ~71% and 73% carry-forward — healthy churn, not over-matching (the question that motivated this milestone now has a running answer on every re-rank). **Still open for Stage 1: the per-application axis** — extend the candidate detail page with trace metadata (tokens/cost/model/version) on the existing screening + essay sections, and add the missing dimension-scoring trace.
