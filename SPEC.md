...
#### Committee-Suggested Criteria (favourite + propose)

A fast-follow on carry-forward that lets the committee *steer what discovery looks for* without violating "AI discovers what varies; the human decides what matters" — proposing an axis is asking the model to *check whether the pool varies on it too*, still discovery, not a weighting. Two seed types feed the next Rank's discovery prompt as **"strongly consider, but you decide"**:

- **Favourited** — an existing dimension the committee stars (★ on each criterion in the accordion) to keep across re-runs. Sent to discovery as its current name + definition. Persists on `criteria.favourited_keys`, carried forward by key (matched dimensions adopt the prior key, so a favourite survives a re-run).
- **Proposed** — free text a member writes ("school-age kids who'd use the playground"). Persists on `criteria.proposed_dimensions` (survives reload), **consumed** by the next run (it becomes a real dimension), then cleared.

The model may refine wording, split one request into several, merge overlapping ones, or **skip** a requested axis if the pool genuinely doesn't vary on it. It flags every dimension it creates from a request with `PoolDimension.from_committee_request`, which the run uses to **auto-favourite** those dimensions — so a written-out axis is protected going forward without a manual star, and the flag survives the model renaming/splitting it (which a text-match never could). Discovery is therefore **blind except for the committee's seeds**: with no seeds it is fully blind (the original first-run behaviour); seeds steer only the axes the committee named. Persistence is `PUT /screening/seeds`; the Rank confirmation card notes how many seeds it will offer. *(Verified against real Bedrock: seeds honored with key-drift handled by the match pass, proposals auto-favourited then cleared, favourites carried forward; the match pass confirmed not over-matching — 11 matched / 7 new on a typical re-run — via `match_audit`.)*

**OPEN QUESTION — surface *all previously discovered* dimensions to choose from, not just this run's? (parked 2026-07-08, to discuss).** Today a member's tier-list chips are the dimensions **this run** discovered (matched ones adopt prior keys, so their tier placement carries forward; unmatched priors simply don't reappear). The `all_known_dimensions` set — every dimension ever discovered across runs, one per key — already exists (it's the match pass's history board), so surfacing it as a menu is cheap to build. The tension to work through when we discuss it:
- **For:** a member could re-add an axis that fell out of this run's discovery ("we cared about co-op governance experience last time and it didn't resurface — put it back"); the pool genuinely may still vary on it even if the model didn't name it this pass. It also makes the committee's accumulated intent visible rather than silently dropped.
- **Against:** it breaks the current clean story that chips describe *this pool's* variation. A dimension from a prior pool may not vary in the current one (its scores would be flat/meaningless), and re-adding it means **scoring it fresh** (cache miss — it wasn't in this run's report), so it's not free. It also risks the committee anchoring on a stale rubric instead of reading what the current pool actually surfaces — the exact "generic ideal member" failure discovery is built to avoid.
- **Middle options to weigh:** show dropped-but-previously-tiered dimensions as a *suggestion* row (not silently gone, not auto-scored) rather than the full history; or lean on the existing **propose** seed instead (a member re-proposes the axis by name, and discovery decides afresh whether the pool varies on it — which keeps the "AI decides what varies" invariant intact). This last is why the feature may be redundant: propose already covers "make discovery reconsider axis X," just without a one-click menu.
- **Leading candidate to explore first — discover-blind-then-reconcile (Jeff's framing, 2026-07-08).** Keep the blind discovery pass *pure* (it commits to what THIS pool varies on, uncontaminated by prior intent), then run a **separate reconcile step**: show the model the prior dimensions that did *not* resurface and ask "does this pool also vary on any of these?" It threads the needle both parked options miss — the committee's accumulated intent gets a second look, but the **model, not the member,** decides whether a resurrected axis genuinely applies to the live pool (preserving the core invariant; no anchoring on a stale rubric). Mechanically it's **the match pass run in reverse** — match asks "is this new dim the same as a prior one?"; reconcile asks "here are priors that didn't reappear — do any hold here anyway?" — so same once-per-rerank economics and model tier. **Two things to design against:** (1) *rationalization pressure* — showing a model "what about these?" biases it toward yes (the same overconfidence the match pass showed on drifted concepts), so the reconcile prompt needs the STRONGER discovery discipline: only revive an axis if the pool genuinely varies on it, a flat axis is a no, and **no is the expected answer for most**; (2) anything revived still scores fresh (cache miss), so it's not free. Advantage over propose: the member need not know the history exists — the system re-checks it automatically and only ever surfaces axes the model re-confirmed against the current pool.
- **Possible first multi-agent workflow (Jeff, 2026-07-08).** The discover→reconcile shape is a natural fit for a small multi-agent orchestration, and a good candidate for the project's first: a discovery agent produces the blind report; one (or several, per prior axis) reconcile agents independently judge "does the pool vary on this prior dimension?" against the same pool; an adversarial/verify step guards the rationalization risk (a skeptic agent that defaults to "no" and must be convinced). Worth weighing against a single deterministic reconcile call — the multi-agent version buys independence and a built-in check on over-revival, at more tokens and orchestration cost. Decide the shape when we take this up; noting it here so the workflow angle isn't rediscovered from scratch.
