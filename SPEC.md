...
3. **Surface weak spots, not just top drivers** (no AI — pure presentation). **Done — solved at the root, where it actually bit.** The original symptom was misdiagnosed as a *selection* problem (top-3-by-`weight × score` hides lows) but turned out to be a *labelling* one: each contribution line printed the dimension's **confidence**, not its **score**, and coloured it on the confidence ramp (high→green). So a candidate with *no* co-op experience showed "Residential Co-op Experience (High)" in green — a confidently-identified *weakness* reading as a strength. The fix shows the **score band** (`Strong ≥0.66 / Moderate ≥0.33 / Weak <0.33`, via `scoreBand()` in `App.tsx`) coloured by merit (strong→green, weak→amber), so a weak dimension now reads "(Weak)" in amber and a weak candidate's whole row honestly reads weak. That's what "prioritise weaknesses as well as strengths" needed: the row no longer disguises low scores. The top-3 are still chosen by `weight × score` (the dimensions that most drove this candidate's position — the right "why here" set), but each is now labelled by its true score, so a heavily-weighted dimension scored low surfaces as the amber strike it is. The detail page already showed the score (a bar) and confidence as *separate* fields, so it never had the conflation; no ordering change was needed there. Surfaced from the end-to-end test that first read as "no low scores in the list" — the lows were always there (44 low-confidence, 115 score<0.34 of 377), just mislabelled.
...
The next milestone is **10: Google Docs report generation**. M9 fast-follow status: (1) tier carry-forward on re-rank — **functionally done (Phases 1–3 + fixes), verified end-to-end**; only the Phase 4–5 cost optimization remains; (2) add-a-dimension mid-tiering; (3) surface weak spots in the ranked list — **done** (the ranked row now labels each contribution by its score band coloured by merit, not by confidence — fixing the bug where a confidently-low dimension showed green "High"); (4) AI Criteria Coach.
