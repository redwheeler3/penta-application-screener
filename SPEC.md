...
>
> **Scoped first step (behavior-neutral observability):** (1) preserve the exception type instead of `str()`-flattening it — add `error_type` to `PassResult`, sourced from the exception class (fully-qualified preferred), bringing screening to parity with the ranking path; (2) emit it durably on the error path in both the screening (`analysis.py`) and ranking-criteria (`ranking.py`) paths. ~20–30 lines, no migration, no behavior change. After a few real runs the failure-mode distribution is known, and *then* the retry decision is evidence-based (the leading candidate being a narrow retry of Pydantic schema-validation failures — nondeterministic and often parse on a second attempt — explicitly NOT throttling, which boto3 already owns).
>
...
**Surface: dev harness first, then in-app admin.** Phase A is a developer-facing eval/metrics harness (pytest-style and/or a CLI, runnable against real or recorded outputs) — that's what unblocks prompt tuning and regression-catching, the immediate pain. Phase B surfaces cost, run history, operational metrics, and the match-audit viewer in an in-app **admin** view (these are operator/admin concerns, consistent with settings being admin-bound later). Recorded-output fixtures matter: evals should be replayable against captured real responses so the suite runs in CI without burning Bedrock spend on every run — which is why structured outputs and prompts are already preserved per call.
