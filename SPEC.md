...
Household income asks for yearly before-tax gross income for applicant, co-applicant, and total household. Gross income includes employment and self-employment, investments including capital gains, social assistance/government benefits/pension, support payments, rental income, and RRSP income.
...
Applicant and committee Google entry instead say **Stay signed in on this device when using Google**
so the choice is visibly scoped to the adjacent Google action. An email-link choice appears
...
[ ] Stay signed in on this device when using Google
...
[ADR 0014](docs/adr/0014-multi-provider-model-routing.md) and the Bedrock Runtime migration is in
[ADR 0016](docs/adr/0016-global-openai-bedrock-runtime.md).
...
4. Store a narrowly scoped Fly deploy token as the watchdog's secret. Bound each Fly API call with a short client deadline, retry a failed read-only Machine lookup once before alerting, and never repeat an ambiguously failed restart request inside the same poll. Derive the target from the app's Machine list rather than hard-coding a Machine ID, and never log tokens, application data, or full API responses.
