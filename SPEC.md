...
- **M14 follow-up — a second cleanup pass.** The M14 cleanup (dead-code removal, de-duplication, the router/module splits, the API-contract and DB-schema rationalization, the docs reduction) landed the big wins, but a further pass at **simplification, readability, and best-practices** is wanted — the first pass was close enough to the code to miss things a fresh look will catch. Scope to define at the time, but likely candidates: over-abstractions or awkward seams introduced by the splits themselves; residual naming/structure drift; spots where a simpler shape reads better than what's there now; and any best-practice gaps (typing, error handling, test structure, component boundaries) surfaced by re-reading with fresh eyes. Same governing rules as M14 — behavior-preserving, tree green each step, rule-of-three (over-abstraction is as bad as sloppiness), no backward-compat when it fights readability. Not yet scheduled; can slot before M15 or run alongside it.
