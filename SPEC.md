...
Remaining: Phase 3 (new-dimension badging in the tier-list UI, reading `newDimensionKeys`), Phase 4 (per-dimension score reuse — per-dimension cache rows + one batched call per candidate, keyed by the match mapping), Phase 5 (estimator + confirmation updates for the match-pass cost and reduced scoring).
