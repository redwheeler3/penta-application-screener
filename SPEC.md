...
- *Phase 4 (per-dimension score reuse) — DONE.* Scores are cached at **per-(candidate, dimension)** granularity, keyed by the dimension **key**, so a dimension that carries forward across a re-rank **reuses its cached score** instead of re-scoring the whole pool. The key insight (a deliberate simplification over an earlier opaque-"lineage-id" design): the cache and the tiers can share *one* identity — the key — if matched dimensions simply **adopt the prior key**. The pieces:
  ...
  - *Shared concurrency core.* The thread-pool / session-on-main-thread / error-isolation mechanism was extracted from `screen_applications` into `run_in_pool`; `screen_applications` stays the 1:1 wrapper for quality flags + essays (behavior unchanged), and scoring uses the same core without bending the 1:1 abstraction. (The subtle session-safety discipline now lives in one place.)
...
12. Database schema refactor — clean up the persisted data model now that the API contract is settled: drop the legacy nullable-for-old-rows columns (`ApplicationAIResult.prompt_version`, `SyncRun.settings_fingerprint`), and rename `ScreeningRun` → `RankingRun` to match the renamed `/ranking` API. Renaming the model frees the word "screening", which then lets `/quality-flags` → `/screening` land to match the UI's Screen step — the M11 rename deferred this to avoid an interim model/route collision. (`/sync` stays `/sync`; see "API Redesign".)
...
- AI passes: `app/ai/pattern_discovery.py`, `app/ai/dimension_scoring.py`, `app/ai/dimension_matching.py` (the carry-forward identity-match pass), `app/ai/essay_analysis.py`, `app/ai/quality_flags.py`, `app/ai/applicant_facts.py` (shared facts view). Schemas in `app/ai/schemas.py` (incl. `DimensionMatch`/`DimensionMatchReport`).
