...
3. **Surface weak spots, not just top drivers** (no AI — pure presentation). The ranked list currently shows each candidate's **top 3 contributions by `weight × score`** — by construction their *strongest* dimensions, so a heavily-weighted dimension the candidate scores *low* on is hidden, even though it's exactly the kind of strike a committee should see. Change the ranked list to also surface notable weak spots (e.g. the most heavily-weighted dimension(s) where the score is low), so a row tells both why a candidate ranks high *and* where they're weak. **This must change the candidate detail page's dimension ordering to match** — the two views should tell the same story about a candidate, so the detail page's "fit dimensions" ordering (today sorted by `weight × score` descending — see `_dimension_scores` in `applications.py`) needs to reflect the same strong+weak logic rather than diverging from the list. Surfaced from this end-to-end test: a freshly-tiered/uniform-weight run showed *no* low scores in the list because top-3-by-`weight×score` always selects highs; the stored data had plenty of lows (verified: 44 low-confidence, 115 score<0.34 of 377). Not a scoring regression — a display choice worth revisiting. Do after carry-forward is committed; it's orthogonal to the carry-forward machinery.
