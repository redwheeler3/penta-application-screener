...
- *Phase 4 (per-dimension score reuse) — DONE.* Scores are cached at **per-(candidate, dimension)** granularity, keyed by the dimension **key**, so a dimension that carries forward across a re-rank **reuses its cached score** instead of re-scoring the whole pool. The key insight (a deliberate simplification over an earlier opaque-"lineage-id" design): the cache and the tiers can share *one* identity — the key — if matched dimensions simply **adopt the prior key**. The pieces:
  ...
  - *Cache shape.* Scoring rows are **one-per-(candidate, dimension)** under `dimension_scoring:<dimension_key>`, each storing a single `DimensionScore`. (`dims_hash`/`dimensions_hash()` from the original whole-set design are deleted.) Cache identity is the key, **not** the definition text — the match pass vouches a carried-forward dimension is the same intent; a *future* "edit a definition" feature would change the key to force a re-score. **Accepted corner (documented):** if discovery re-coins an old key string for a genuinely *different* concept that the match pass does *not* match, the new concept would read the old one's cached score. Low-probability and dev-DB-resettable; chosen over carrying a separate identity layer to prevent it.
