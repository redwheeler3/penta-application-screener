...
1. Data-model split, sliced into four independently-shippable, separately-migrated steps (the maps confirmed the `RankingRun` write side funnels through 4 service functions, but ~10 "BOTH" functions join member-view to shared-analysis, and `status == ELIGIBLE` is the pool filter in four independent places — so slicing keeps each migration small and round-trip-verifiable):
   ...
   - **1b — `RankingRun` → `Analysis` + `MemberRanking`.** The choke-point split; `get_current_analysis()` replaces `get_current_run`. Per-member reads resolve via `require_current_user` (one logged-in member for now — forward-compatible, no placeholder). A member's tier view of an analysis they didn't tier is materialized lazily by `get_or_create_member_ranking` (named for the write), seeded by carrying their prior tiers forward — **pure JSON, no AI/Bedrock**. Tier/seed saves take the **viewed `analysisId`** and the server returns `409 stale_analysis` if it isn't current (inert at one member; makes the endpoint honest — "tier *this* analysis", not "whatever's latest" — so the phase-2 refresh UX is not a bolt-on). **Deferred to phase 2 (needs real concurrency to exercise):** the "this ranking was refreshed by another member — reload" banner + auto-refetch when a save 409s.
   - **1c — per-member eligibility + union pool** (absorbs the former phase 2, by decision 2026-07-24 to pull it forward). Eligibility becomes per-member via **compute-on-read**: `MemberEligibility` stores ONLY a member's human overrides (sparse — one row per actual override), and effective eligibility for `(member, applicant)` = *their override if present, else the computed machine verdict* (`resolve_machine_status` over the applicant's `hard_filter_reasons` + cached screening flags). So `status`/`status_source`/`reviewed_fingerprint` **leave `Application`** (they were pure derivations that happened to be stored) — import/screening keep writing ONE shared machine baseline (`hard_filter_reasons` + the flag cache); no per-member write-fanout, no lazy machine-row materialization. The four pool filters (`pool_fingerprint`, discovery `eligible_applications`, scoring `applications_to_score`, screening `applications_for_screening`) become the **union**: eligible for ≥1 member. Per-member **cache-gap staleness**: a member's "re-rank needed" fires when their eligible view references an applicant not yet in the shared analysis (ADR 0011). Migration backfills just the ONE existing human override (compute-on-read means nothing else needs storing); verified on a copy. 1d then makes the *computed* verdict diverge per member with zero rework of `MemberEligibility`.
   - **1d — settings split (numeric rules per-member) + tab reorg.** The eligibility RULE fields split off the shared `app_settings` blob: income/age/children/`disabled_rules` become per-member via **compute-on-read** (`evaluate_hard_filters(normalized, member_rules)` runs on read — so stored `hard_filter_reasons` is dropped and import just normalizes), backed by a shared **committee-default** rules row + a sparse copy-on-write `member_rules` (a member's row exists only once they diverge; else they read the default). This is what makes 1c's computed machine verdict finally *diverge* per member — the rules half of the machine verdict is now per-member (the `union_eligible_application_ids` "shared machine verdict" assumption from 1c is revisited here). Infra config (`google_sheet_id`, all of `AISettings`) stays the one shared row. **Import staleness:** `settings_fingerprint` hashes the committee-default rules + sheet id (a member's private divergence doesn't amber Import — it recomputes on read, free). **Pets are the exception, deferred to 1e:** pet limits (`max_dogs`/`max_cats`/`allow_other_pets`) live in the screening *prompt* + its cache-version hash, not `RulesConfig` — so they stay on the committee default feeding the shared screening pass (cache + `screening_prompt_version` stay single) in 1d. **Tab reorg:** Settings → **Eligibility Settings** (member; funnel) + **Admin Settings** (admin; gear; Access allowlist as a subtab); **Observability + Evals gated admin-only**.
   - **1e — pet policy as deterministic per-member facts (option B). ✅ built (code/tests/migration); eval goldens re-baselined structurally, pending a real-Bedrock validation run (see "Validation Experiments Owed").** Pets used to be judged inside the shared screening prompt (a `pet_policy` *flag*), so making them per-member was a screening-pass redesign, not a settings move — its own slice. Pet judgment moved out of AI-arithmetic: the shared screening pass now EXTRACTS neutral pet *facts* and a per-member deterministic hard filter decides pass/fail, so pet limits diverge per member without fragmenting the shared screening cache. **The one design decision:** pet facts are screening OUTPUT (the model turns free-text `pets_text` → counts — that can't be deterministic), so they can't live in `normalized` (a pure function of `raw_row`). They ride on `ScreeningReport.pets`, and `evaluate_hard_filters(normalized, rules, *, pet_facts=None)` takes them as an OPTIONAL param — `None` (import; the screening-eligibility gate) skips the pet check, so pets never gate WHO gets screened (they can't; we need the screen to get the facts); the on-read eligibility path loads the screening result and passes them in, so pets gate per member there. As built:
     1. **Screening prompt** (`app/ai/screening.py`): dropped `_pet_policy_line` + the `{pet_policy}` interpolation; `screening_prompt_version()` is now argless (pure function of prompt text — a pet-limit change no longer invalidates the screening cache, it's a hard-filter change judged on read). The prompt now instructs neutral extraction ("How to extract pets"). `ScreeningReport` gained `pets: PetFacts` (`dogs`/`cats`/`other_pets`); the `pet_policy` `FlagCategory` was removed (pets are never a flag).
     2. **Hard filter** (`app/domain/hard_filters.py`): `RulesConfig` gained `max_dogs`/`max_cats`/`allow_other_pets` (+ `DEFAULT_*` constants) and a domain `PetFacts`; `_pets_over_limit` emits one `pets_over_limit` reason per violated category. `services/rules.pet_facts_from_screening` lifts facts off a cached result; `services/eligibility.pet_facts_by_app` (sibling to `machine_flags_by_app`) batch-loads them; all on-read sites (`applications`, `dashboard`, `eligibility` union + per-member) thread `pet_facts` through.
     3. **Settings move:** pet fields moved `AppSettings` → `EligibilityRules` (per-member; committee default + copy-on-write). `AppSettings` is now pure infra (sheet + AI). `settings_fingerprint` now hashes the committee-default pet limits (a default pet-limit change reclassifies import). Frontend: pet inputs moved Admin Settings → Eligibility Settings; `pet_policy` flag label removed; `pets_over_limit` added to `REASON_FIELDS`. Migration `f6a7b8c9d0e1` moves the pet keys `app_settings` → `committee_default_rules` and backfills existing `member_rules` rows (Model A: a diverged member keeps a complete ruleset). Verified round-trip on a copy; live DB migrated (backup `data/penta_screener.pre-M15-1e.db`).
     4. **Re-baseline:** the screening eval harness now grades extracted pet FACTS (`expected_pets` on a case: dogs/cats exact, each other-pet noun present), not a `pet_policy` flag; goldens rewritten to extraction cases (2-dogs-1-cat, exotic velociraptor→other, rabbit→other, negation→zero, both-present baseline). Deterministic pet-filter unit tests + an end-to-end per-member pet-divergence eligibility test are committed and green. **Still owed:** a real Bedrock screening run to confirm the model reliably extracts pet counts from free text and to tune the prompt/expected values from what it actually produces (mock proves only plumbing). Needs Jeff's real-run go-ahead.
   Pet limits are now per-member and the rules split is complete.
   - **1f — admin-editable committee defaults + member reset-to-default. ✅ built.** Both gaps 1d left are closed. **Backend:** `DELETE /eligibility-rules` (member reset — `reset_member_rules` drops the copy-on-write row, idempotent, single-row delete per Model A) returns the now-effective default (`is_default` True); `GET /eligibility-rules/committee-default` (any member — the baseline + the diff reference); `PUT /eligibility-rules/committee-default` (`require_admin` — edits the shared default via `save_committee_default_rules`). **Frontend:** member Eligibility Settings shows a lazy "compared to committee default" divergence diff (`DivergencePanel` — differing fields only, computed on read from the live draft vs the current default) + a "Reset to committee default" button; admin gets a **Committee Defaults** subtab in Admin Settings (`CommitteeDefaultsPanel`, self-fetching, own save). Verified live: an admin edit does NOT touch a diverged member's row (Model A, no fanout — `test_admin_default_edit_does_not_touch_diverged_member`); reset flips a member back to `isDefault` and the diff/panel clears. No migration (behavior only; the `committee_default_rules` row already existed from 1d).
     - **Divergence model — whole-ruleset fork (Model A), decided.** A member's `MemberRules` row stays one JSON blob holding the *complete* ruleset — a coherent set they reviewed as a whole, not a sparse per-field patch. Rejected the per-field-override alternative (Model B: store only changed fields, unspecified fields fall through to the *live* default) because it lets an admin default-change silently mint income/age/children combinations no member ever reviewed — and an unreviewed combination is exactly the bad outcome for a screening decision. It also re-introduces the committee default as an authority that reaches into members' private views, the opposite of M15's per-member-isolation thesis (the default is a *seed*, the member's list is sovereign). Consequences: **an admin editing the defaults has zero side effects on member rows** — no reconciliation, no write-fanout (the thing 1c's compute-on-read was chosen to avoid); a diverged member keeps their rules until *they* reset; Reset is a single-row `DELETE`, already atomic (no per-field rows needed).
     - **Reset shows a lazy divergence diff (display-only).** When a member has their own rules, the Eligibility Settings page shows a passive "compared to committee default" panel — field-by-field *their value → default value*, only for fields that differ — so "Reset" is never scary. Computed lazily on read (their frozen blob vs the *current* default); no stored reconciliation, no admin-side trigger. The only mutation is the member clicking Reset.
     - **No default-version stamp** (considered, dropped). Model A's one genuine weakness is silent drift — a member who forked never learns the committee later changed the default. A version/timestamp on the default + a "the default changed since you personalized" nudge would surface it, but we're not building it: the divergence diff already shows *current* default values whenever the member looks, so a curious member sees the delta; the added row field + nudge UI isn't worth it for a small committee.
   - **1g — restore the two-phase eligibility mental model after pets (design locked 2026-07-24; not yet built).** After 1e made pets a deterministic per-member rule, they surfaced with a **"decided by Rules"** badge — but a pet verdict needs the AI to read the free-text pets field first, so it can only land at **Screen**, not **Sync**. That made the workflow read *"some rules resolve at Sync, one more rule shows up later"* — the appears-≠-is trap. The fix is NOT to relocate any computation (eligibility is compute-on-read and per-member, so there is no single verdict to "run" or store at a step — trying to would re-introduce the write-fanout/stored-verdict mess 1c deleted). The fix is to put pets in the honest bucket. Three moves, sequenced:
     - **Move 1 — pet verdict → status source `AI`, not `RULES`. ✅ built.** `resolve_machine_status` (`app/domain/status.py`) now takes the reason *codes* (not a `has_reasons` bool) and partitions: any **non-pet** reason → `RULES` (Sync-knowable, high trust); else a `pets_over_limit` reason OR an AI flag → `AI`; else eligible. `PETS_OVER_LIMIT_CODE` is a named constant in `hard_filters.py` (single source of truth, imported by `status.py`). This restores the two-phase model — **Sync = numeric deterministic rules (income/age/children/real-estate), Screen = everything that needed the model (AI flags + pets)**. Reason TEXT unchanged (detail page still shows "3 dogs; at most 1 allowed"); **eligibility outcomes unchanged** (a pet violation still makes ineligible — union pool / eligible sets / staleness fingerprint untouched); only the source badge moves, and only for a pet-ONLY deterministic verdict (mixed income+pet stays `RULES` — the income reason made it ineligible at Sync anyway). Frontend `SOURCE_DESCRIPTIONS.ai` widened to "a flag it raised or the pet count it read" so a pet-ineligible badged AI doesn't imply a flag that isn't there. The detail page's "Filter reasons" section still lists the pet reason (pets *are* a deterministic filter structurally); the AI badge explains provenance — the two are different axes, deliberately not split. Tests: resolver unit tests (pet-only → AI, mixed → RULES) + end-to-end `effective_status_for` tests in `test_eligibility.py`.
     - **Move 2 — members can toggle the AI checks on/off (not just the deterministic rules). ✅ built.** A member can now mute any AI screening check (the 9 flag categories AND pets) the same way they mute a deterministic rule — a muted check is **hidden AND non-gating** for that member (matching how a disabled reason already behaves), others' lists unaffected. Mechanism: `active_flags(flags, disabled_checks)` in `services/eligibility.py` (the flag analogue of how `evaluate_hard_filters` drops disabled reason codes) applied at every member-facing site (`applications` list/detail/override, `dashboard` tallies, `eligible_application_ids_for`). `None` (unscreened) passes through as `None` so "unknown" stays distinct from "screened-clean". **Union pool:** the old "any flag → ineligible for everyone" shortcut moved *inside* the per-ruleset loop (an app is machine-eligible under a ruleset iff no hard reason AND no active flag under that ruleset's `disabled_checks`); grouping key is unchanged because `disabled_checks` is already part of `RulesConfig`, so members with different mutes are already distinct rulesets — non-quadratic as before. Browser-verified live: muting "Suspicious contact info" flipped a `fake_contact`-only applicant to eligible/untouched with no AI re-run, while an `internal_inconsistency` applicant stayed ineligible.
     - **Move 3 — rename `disabled_rules` → `disabled_checks`. ✅ built.** One flat set spanning both deterministic reason codes and AI flag categories; the two namespaces are disjoint so each filter ignores the other's strings. Renamed across `RulesConfig` / `EligibilityRules` (wire `disabledChecks`) / `settings_fingerprint` / frontend. Migration `b8c9d0e1f2a3` renames the key in the stored `committee_default_rules` + `member_rules` blobs (round-tripped on a copy; applied live). **Toggle UI:** `EligibilitySettingsPanel` now renders two labeled groups — **Deterministic rules** and **AI screening checks** — both editing the one flat `disabledChecks` list (`DETERMINISTIC_CHECKS` / `AI_CHECKS` catalogs in constants). **Pets are listed in the AI group** (they present as AI everywhere — badge, reason cell, source — so the toggle matches). Detail-page panel convergence (pets rendered as AI-style evidence cards alongside flags) is a small remaining display polish, not yet done — logged below.
     - **Detail-page panel convergence + pet reasoning. ✅ built.** The detail page now groups findings by *source* to match the badge: a **Deterministic rules** panel (non-pet hard reasons) and an **AI screening** panel (flags AND pet findings, pets rendered as evidence cards alongside flags). The two backend pipelines (evidence-carrying flags vs threshold reasons) stay separate — deliberately not merged (flags carry evidence, threshold reasons don't; forcing both through one shape is over-abstraction). To give a pet card real evidence — the analogue of a flag's cited summary — the screening extraction gained a neutral `reasoning` field (`PetFacts.reasoning`): the AI's read on what the pets field states and how it classified it into the counts (describe/classify only, never a policy verdict — the counts are what the deterministic rule judges). Surfaced on the detail as `PetFactsOut.reasoning` (`_pet_facts_out` lifts it from the screening output); the card shows it, falling back to the raw pets field only for pre-`reasoning` results. **Prompt changed → `screening_prompt_version` bumped → needed a real re-screen to populate `reasoning`; done.** (Also trimmed the bloated pet-extraction prompt line.) The reasoning initially named the internal `pets_text` field because the prompt did — fixed by rewording the pet-extraction line + `reasoning` field description to plain human terms (condensed, no added guardrails); takes effect on the next re-screen (`screening_prompt_version` bumped again).
...
### Future UX Enhancements (M15-era, deferred)

Usability polish raised while building M15, parked so they aren't forgotten. The common thread: make the app legible to a member who does NOT know the system the way we do.

1. **Nudge a member toward a re-rank after they propose a dimension.** When a member proposes a new dimension, their proposed axis does nothing until a Rank runs (discovery has to fold it in). A member who doesn't know that sees no effect and assumes it's broken. Surface the pending state and move them toward the action: e.g. the **Rank button turns amber/pending** and its confirmation card leads with **"Discover new criteria"** as the primary action, ideally naming that this run will introduce their proposed dimension. Open to a smoother interaction than a button color + confirm if one reads better — the requirement is that a proposal visibly implies "re-rank to apply," not the specific mechanism.
2. **In-app feedback mechanism (member → admin).** A member can submit free-text feedback from any page; it surfaces to the admin (a feedback surface in Admin Settings, alongside Access/Configuration). Capture enough context to act on it without a back-and-forth: who submitted (user id/email), what page/route they were on, and diagnostic context (e.g. current analysis id, active tab, app version/commit, timestamp, maybe viewport). Purpose is Jeff improving the app from real member friction. Treat feedback text as potentially sensitive (a member may paste applicant specifics) — same PII posture as the rest of the app.
