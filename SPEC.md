...
- **No-op Screen re-runs now blocked too (symmetry).** Screen had the same shape — a fully-cached re-run was a $0 no-op — but only the UI avoided it; the API still allowed it. Made symmetric with Rank: `/quality-flags/run` blocks (409) when `to_analyze == 0` (nothing uncached), and the UI shows the same "already up to date" toast. Screen needs no pool-fingerprint (its per-application cache already tells it what is uncached); the gate is simply "is there anything new to analyze." Both steps now behave identically — a no-op is blocked server-side, surfaced as a toast client-side. Note the human-sticky-status guarantee (a re-run never overwrites a human override) is now exercised on *partial* re-runs: a cached human-overridden application still flows through the status hook when a new applicant triggers the run.
