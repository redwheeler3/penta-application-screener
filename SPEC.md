...
>
> **Still open for Stage 1:** (a) **discovery is not yet surfaced** — the Insights tab shows only the *matching* half of the run-level axis; the raw discovery report (what pattern discovery emitted, its summary + full dimension set) still needs a panel there. (b) **the per-application axis** — extend the candidate detail page with trace metadata (tokens/cost/model/version) on the existing screening + essay sections, and add the missing dimension-scoring trace.
