...
- **89d3710 — Fly deploy scaffolding + prod auth hardening.** Added `fly.toml`, Dockerfile, and the initial push-triggered workflow; that workflow was later removed so production releases remain the explicit `fly deploy --remote-only` operator action. `https_only` cookie derives from the `frontend_url` scheme; `SESSION_SECRET`, Google client, and AWS keys remain Fly secrets rather than image content. Bedrock uses a static invoke-only IAM key because Fly cannot assume an AWS role.
